...
var sendAddPaymentInfoEvent = require('../sendStartTrialEvent');
var getFbQueue = require('../../helpers/getFbQueue.js');

describe('Send Start Trial Event module', function () {
  setupTests.setup();
  ...
  test('logs message to turbine', function () {
    ...
    expect(turbine.logger.log.mock.calls[0]).toEqual([
      'Queue command: fbq("track", "StartTrial", undefined) with' +
        ' eventId: 11111.'
    ]);
  });
});
