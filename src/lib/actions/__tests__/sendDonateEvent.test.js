...
var sendAddPaymentInfoEvent = require('../sendFindLocationEvent');
var getFbQueue = require('../../helpers/getFbQueue.js');

describe('Send Donate Event module', function () {
  setupTests.setup();
  ...
  test('logs message to turbine', function () {
    ...
    expect(turbine.logger.log.mock.calls[0]).toEqual([
      'Queue command: fbq("track", "FindLocation", undefined) with' +
        ' eventId: 11111.'
    ]);
  });
});
