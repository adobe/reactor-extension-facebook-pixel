...
var sendAddPaymentInfoEvent = require('../sendCustomizeProductEvent');
var getFbQueue = require('../../helpers/getFbQueue.js');

describe('Send Customize Product Event module', function () {
  setupTests.setup();
  ...
  test('logs message to turbine', function () {
    ...
    expect(turbine.logger.log.mock.calls[0]).toEqual([
      'Queue command: fbq("track", "CustomizeProduct", undefined) with' +
        ' eventId: 11111.'
    ]);
  });
});
